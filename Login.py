...
def main(state):
    st.write(f"You're logged in as {state.get_user_email()}")
    st.button("Logout", on_click=lambda: state.logout_user())
        ...
        try:
            ...
            try:
                ...
                if token.is_expired():
                    ...
                    state.set_user_email(user_email)
                    main(state)
    else:
        main(state)
